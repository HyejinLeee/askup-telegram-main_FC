...
import hupper
from gpt_util import chatgpt_callback_response, chatgpt_response, get_search_info
...
async def askup_04_memory(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if response.get("function_call"):
        ...
        put_message_list(
            user_id=user_id,
            message_list=[new_message, response],
        )
        ...
        await update.message.reply_text(function_response)



        function_message = {"role": "function", "name": f"{function_name}", "content": f"{function_response}"}

        put_message_list(
            user_id=user_id,
            message_list=[function_message],
        )



    else: 
        logging.info("normal chatting")
        await update.message.reply_text(response.content)

        # Update DB
        put_message_list(
            user_id=user_id,
            message_list=[new_message, {"role": "assistant", "content": response.content}],
        )
